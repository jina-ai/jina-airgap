...
No server, no network, no model weights -- numpy is the only dependency. What
it pins down is the pure rendering, where a regression is silent; the adapter
*response* bodies need a running image and are out of scope here.
...
def test_every_contract_holds():
    """The checks above run at import; this is what pytest fails on."""
    failed = len(_results) - sum(_results)
    assert not failed, f"{failed} of {len(_results)} contract checks failed"
